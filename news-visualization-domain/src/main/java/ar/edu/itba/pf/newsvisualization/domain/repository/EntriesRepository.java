...
public interface EntriesRepository extends CrudRepository<Entry, String> {
    ...
    @Query(value = "SELECT COUNT(*), e.seccion, e.tmp FROM entries e WHERE e.fecha between ?1 and ?2 group by e.seccion, e.tmp")
    List<Object[]> countByDateAndMediaAndCategories(Date from, Date to);
