...
public interface EntriesRepository extends CrudRepository<Entry, String> {
    ...
    @Query(value = "SELECT COUNT(*), e.seccion, e.tmp FROM entries e WHERE e.fecha between ?1 and ?2 and e.seccion = ?3 group by e.seccion, e.tmp")
    List<Object[]> countByDateAndMedia(Date from, Date to, String seccion);
